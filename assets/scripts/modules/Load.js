...
export default class Load extends Module {
  ...
  static HEADER_HEIGHT = 6.25; // in rem
  static BASE_REM = 16;
    ...
    this.load.on('loaded', (transition, oldContainer, newContainer) => {
      ...
      switch (transition) {
        case Load.TRANSITION_ABOUT:
          this.call(
            'scrollTo',
            { target: '#about-us', immediate: true, offset: this.headerHeight * -1, force: true },
            'Scroll'
          );
          break;
        case Load.TRANSITION_BLOCK:
          this.call(
            'scrollTo',
            { target: '#block', immediate: true, offset: this.headerHeight * -1, force: true },
            'Scroll'
          );
          break;
        case Load.TRANSITION_REVIEWS:
          this.call(
            'scrollTo',
            { target: '#reviews', immediate: true, offset: this.headerHeight * -1, force: true },
            'Scroll'
          );
          break;
        case Load.TRANSITION_BOXES:
          this.call(
            'scrollTo',
            { target: '#boxes', immediate: true, offset: this.headerHeight * -1, force: true },
            'Scroll'
          );
          break;
        default:
