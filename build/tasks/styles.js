 ...
 */
export const developmentStylesArgs = [developmentSassOptions, developmentPostCSSOptions, false];
export const productionStylesArgs = [productionSassOptions, productionPostCSSOptions, true];
...
export default async function compileStyles(
  ...
  } else if (sassOptions !== developmentSassOptions && sassOptions !== productionSassOptions) {
   ...
   * @return {Promise}
   */
  loconfig.tasks.styles?.forEach(async ({ infile, outfile, label = null }) => {
    ...
    try {
      ...
      if (supportsPostCSS && postcssOptions) {
        if (typeof postcssProcessor === 'undefined') {
          postcssProcessor = createProcessor(postcssPluginsMap, postcssOptions);
        }
