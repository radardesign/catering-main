 ...
 */
export const developmentSVGsArgs = [developmentMixerOptions];
export const productionSVGsArgs = [productionMixerOptions];
...
export default async function compileSVGs(mixerOptions = null) {
  ...
  } else if (mixerOptions !== developmentMixerOptions && mixerOptions !== productionMixerOptions) {
   ...
   * @return {Promise}
   */
  loconfig.tasks.svgs?.forEach(async ({ includes, outfile, label = null }) => {
    if (!label) {
