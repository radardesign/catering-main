 ...
 */
export const developmentScriptsArgs = [developmentESBuildOptions];
export const productionScriptsArgs = [productionESBuildOptions];
...
export default async function compileScripts(esBuildOptions = null) {
   ...
   * @return {Promise}
   */
  loconfig.tasks.scripts?.forEach(async ({ includes, outdir = '', outfile = '', label = null }) => {
    if (!label) {
    ...
    try {
      ...
      if (outdir) {
        ...
        throw new TypeError("Expected 'outdir' or 'outfile'");
      }
